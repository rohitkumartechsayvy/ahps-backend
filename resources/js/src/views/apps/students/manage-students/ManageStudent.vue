  ...
  <div id="page-user-list">
    ...
    >
      <div class="vx-row">
        ...
        <div class="vx-col md:w-1/4 sm:w-1/2 w-full">
          <label class="text-sm opacity-75">Blocked</label>
          <v-select
...
export default {
  ...
  data() {
    return {
      ...
      classOptions: [{ label: "I (A)", value: "I (A)" }],
      ...
      columnDefs: [
        ...
        {
          headerName: "Received Transfer Certificate?",
          field: "transfer_certificate",
          filter: true,
          width: 150,
        },
        {
          headerName: "Received Migration Certificate?",
          field: "migration_certificate",
          filter: true,
          width: 150,
        },
        {
          headerName: "Received Birth Certificate?",
          field: "birth_certificate",
          filter: true,
          width: 150,
        },
        {
          headerName: "Received Marksheet Certificate?",
          field: "marksheet_certificate",
          filter: true,
          width: 150,
        },
        {
          headerName: "Received Aadhar Card Certificate?",
          field: "adhaar_card_certificate",
          filter: true,
          width: 150,
        },
        {
  ...
  computed: {
    usersData() {
      console.log(
        "this.$store.state.studentManagement.students",
        this.$store.state.studentManagement.students
      );
      return this.$store.state.studentManagement.students;
    },
    paginationPageSize() {
  ...
  created() {
    ...
    if (this.$store.state.AppActiveUser.user_type == "teacher") {
      this.$store
        .dispatch("studentManagement/fetchAssignedStudents")
        .catch((err) => {
          console.error(err);
        });
    } else {
      this.$store.dispatch("studentManagement/fetchStudents").catch((err) => {
        console.error(err);
      });
    }
    this.classData();
