...
export default {
  ...
  methods: {
    editRecord() {
      this.$router
        .push(`/student-edit/` + this.params.data.user_id)
        .catch(() => {});
    },
    confirmDeleteRecord() {
      this.$vs.dialog({
    ...
    deleteRecord() {
      ...
      console.log("this.params.data.user_id", this.params.data.user_id);
      /* UnComment below lines for enabling true flow if deleting user */
      this.$store
        .dispatch("studentManagement/removeRecord", this.params.data.user_id)
        .then(() => {
          this.showDeleteSuccess();
        })
        .catch((err) => {
