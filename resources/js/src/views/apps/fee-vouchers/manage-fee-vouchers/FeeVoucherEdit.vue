      ...
      <div slot="no-body" class="tabs-container px-6 pt-6">
        <vs-tabs v-model="activeTab" class="tab-action-btn-fill-conatiner">
          <vs-tab label="Edit Session" icon-pack="feather" icon="icon-user">
            <div class="tab-text">
              <fee-voucher-edit-tab-account class="mt-4" :data="charge_data" />
            </div>
          </vs-tab>
        </vs-tabs>
      </div>
...
<script>
import FeeVoucherEditTabAccount from "./FeeVoucherEditTabAccount.vue";
// Store Module
...
export default {
  ...
  methods: {
    fetch_charge_data(chargeId) {
      this.$store
        .dispatch("feeVoucherManagement/fetchCharge", chargeId)
        .then((res) => {
          console.log(res);
  ...
  created() {
    // Register Module feeVoucherManagement Module
