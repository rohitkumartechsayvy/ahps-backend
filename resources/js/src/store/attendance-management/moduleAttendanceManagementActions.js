...
export default {
  ...
  fetchAttendance(context, sessionId) {
    return new Promise((resolve, reject) => {
  ...
  fetchStudentAttendance({ commit }, studentId) {
    return new Promise((resolve, reject) => {
      axios.delete(`${process.env.MIX_LIVE_URL}api/v1/fetch-student-attendance/${studentId}`, { headers: { "Authorization": `Bearer ${_token}` } }).then((response) => {
        console.log('attendnace response', response.data.response);

        commit('SET_ATTENDANCE_EVENTS', response.data.response)
        resolve(response)
      })
        .catch((error) => { reject(error) })
    })
